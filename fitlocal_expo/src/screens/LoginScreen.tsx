  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      try {
        ...
        // CRITICAL FIX: We are passing 'data.user' here so the Profile Screen gets it!
        onLogin(data.user);
        
      } catch (jsonError) {
        console.error("Server sent weird data:", textResponse);
        setErrorMsg("Server error: Did not receive JSON.");
        setLoading(false);
      }

    } catch (error) {
      console.error("Fetch completely failed:", error);
      setErrorMsg('Network connection blocked. Check your backend terminal.');
      setLoading(false);
    }
  };
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
      ...
      <View style={styles.formContainer}>
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput 
            style={styles.input} placeholder="Email Address" placeholderTextColor="#888" 
...
const styles = StyleSheet.create({
  ...
  inputContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1E1E1E', borderRadius: 12, marginBottom: 16, paddingHorizontal: 15, height: 55, borderWidth: 1, borderColor: '#2A2A2A' },
  icon: { marginRight: 10 },
  input: { flex: 1, color: '#ffffff', fontSize: 16, outlineStyle: 'none' as any },
  loginButton: { backgroundColor: '#FF5722', borderRadius: 12, height: 55, justifyContent: 'center', alignItems: 'center', marginTop: 10, shadowColor: '#FF5722', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 5, elevation: 5 },
